...
\title{Analýza a srovnání architektur REST, GraphQL a gRPC z hlediska výkonu a bezpečnosti}
\title[english]{Performance and Security Analysis of REST, GraphQL, and gRPC API Architectures}
...
\annotation{Práce se zabývá srovnávací analýzou architektonických principů,
 výkonnosti a bezpečnosti komunikačních protokolů REST, GraphQL a gRPC.
  Na základě referenční implementace jsou provedeny benchmarky zaměřené na latenci,
  propustnost, velikost přenášených dat a vytížení systémových prostředků při různých typech zátěže.
  Práce dále analyzuje typické bezpečnostní hrozby jednotlivých přístupů a navrhuje způsoby jejich mitigace. 
  Výsledkem je sada doporučení pro volbu vhodného komunikačního protokolu podle typu aplikace.}

\annotation[english]{This paper presents a comparative analysis of the architectural principles,
 performance characteristics, and security aspects of REST, GraphQL, and gRPC. 
 Based on a reference implementation, benchmarks are conducted focusing on latency,
 throughput, payload size, and system resource utilization under various query types and load levels. 
 The paper also analyzes typical security threats associated with each approach and proposes corresponding mitigation strategies.
 The result is a set of recommendations for selecting an appropriate communication protocol based on application type.}

%% Klíčová slova práce, včetně anglických. Oddělená (obvykle) středníkem.
\keywords{REST; GraphQL; gRPC; architektura API; výkonnostní testování; latence; propustnost; bezpečnost; kontejnerizace; komunikační protokoly}
\keywords[english]{REST; GraphQL; gRPC; API architecture; performance benchmarking; latency; throughput; security; containerization; communication protocols
}
...
% Teoretická část
\section{Webová API a komunikace}
\subsection{Webové API a model klient--server}
\subsection{Protokol HTTP: metody, stavové kódy, hlavičky}
\subsection{HTTP/1.1 vs HTTP/2}
\subsection{Serializace dat: JSON vs Protocol Buffers}
\section{REST}
\subsection{Fieldingova disertace a architektonická omezení REST}
\subsection{Zdroje, URI a reprezentace}
\subsection{Richardson Maturity Model a HATEOAS}
\subsection{Slabiny REST: over-/under-fetching a verzování}
\section{GraphQL}
\section{gRPC}
\section{Bezpečnost API}
\section{Metodika srovnání a výkonnostní testování}
% Praktická implementace
\section{Cíle a návrh experimentu}
\section{Datová vrstva a doménový model}
\section{Implementace tří rozhraní}
\section{Návrh měření a testovací prostředí}
\section{Návrh bezpečnostní analýzy}
\section{N/A}
